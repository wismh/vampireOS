...
- Userland C: `hi`, `sh`, `init`, `fbhello`, CRT stubs, 8×8 font helper, `memcpy` / `strlen` / `strcmp`, freestanding `printf` / `snprintf` (`%s` `%d` `%x`; `write` fd 1). `run hi` prints `hi 42`. Packed `catch` installs SIGINT and sleeps.

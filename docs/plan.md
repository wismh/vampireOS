...
One `vos-N` slice per step. Each slice boots in QEMU and leaves a command or a line on the VGA that was impossible the day before. Kernel pad is 120 KiB (`KERNEL_SECTORS` 240); bump `KERNEL_SECTORS` and `KERNEL_SIZE` together when `.text` plus `.bss` get near the PMM bitmap again. No full libc, no second language — stay on the clang/nasm/lld flags in `CMakeLists.txt`. Grow freestanding helpers only.

## Now

- Volume: FAT12, 1024 data clusters, four sectors per FAT (both copies). 8.3 names plus VFAT LFN (`ls` / `open` / `cat longname.txt`; `put` / `cp` / `>` of a longer name writes LFN + 8.3 alias). Boot PCI-scans for virtio-blk (vendor `0x1AF4`, device `0x1001` or modern `0x1042`), virtqueue-reads LBA 0, and prints `virt 55aa`. FAT reads/writes still use AHCI DMA when a controller probed (PCI class 0x0106, boot prints `ahci 55aa`, first volume write prints `ahci wr`); else ATA PIO. Preference for a later slice is VirtIO → AHCI → ATA; this slice keeps the volume on AHCI/ATA. BIOS still boots from IDE; QEMU attaches the same image to AHCI and to `-device virtio-blk-pci` without `snapshot=on` (`file.locking=off`, `cache=writethrough`).
...
- No job control (`fg` / `bg` / `jobs`), no FAT16, no VirtIO-blk volume I/O (probe-only this slice), no serial console, no SMP, no networking, no signals beyond kill/Ctrl+C.

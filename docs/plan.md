...
42. **Early boot serial** — done: stage 2 inits the 16550 at COM1 (`0x3F8`, 115200 8N1) in real mode (`out dx,al`) and prints `boot` before VBE/`kmain`. QEMU `-serial stdio` (or `-serial file:...`) shows `boot` then `$`. Kernel console mirror stays. No virtio-net.

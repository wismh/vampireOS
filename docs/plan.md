...
- Volume: FAT12, 1024 data clusters, four sectors per FAT (both copies). 8.3 names plus VFAT LFN (`ls` / `open` / `cat longname.txt`; `put` / `cp` / `>` of a longer name writes LFN + 8.3 alias). FAT reads/writes use AHCI DMA when a controller probed (PCI class 0x0106, boot prints `ahci 55aa`, first volume write prints `ahci wr`); else ATA PIO. BIOS still boots from IDE; QEMU attaches the same image to AHCI without `snapshot=on` so those writes persist (`file.locking=off`, `cache=writethrough`).
...
- No job control (`fg` / `bg` / `jobs`), no FAT16, no VirtIO-blk, no serial console, no SMP, no networking, no signals beyond kill/Ctrl+C.

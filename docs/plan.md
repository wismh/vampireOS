...
- Volume: FAT12, 1024 data clusters, four sectors per FAT (both copies). 8.3 names plus VFAT LFN (`ls` / `open` / `cat longname.txt`; `put` / `cp` / `>` of a longer name writes LFN + 8.3 alias). FAT reads/writes stay on ATA PIO. Boot probes AHCI (PCI class 0x0106), DMA-reads LBA 0, and prints `ahci 55aa` (MBR magic) on VGA and the LFB.
...
- No job control (`fg` / `bg` / `jobs`), no FAT16, no AHCI write, no VirtIO, no serial console, no SMP, no networking, no signals beyond kill/Ctrl+C.

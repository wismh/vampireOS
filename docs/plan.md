...
33. **`TASK_MAX` 16** — done: table is 16; scans use `TASK_MAX` / `task_count`. `sleeper &` enough times (or a sixteen-deep `fork`) still schedules; `ps` lists slots ≥ 8. Extra tasks share a VGA overlay row. No tick column.

...
- Syscalls through `int 0x30`: write, exit, yield, sleep, wait/waitpid, open (rsi 1 creates, rsi 2 creates and appends), close, read, readdir, exec, pipe, brk, fork, dup2, lseek, stat, kill, mmap (rsi length rounded up to pages, rdx a file fd copies that file into the mapping), munmap, fbinfo, fbpix, fbpresent, sigaction (27; rdi signo, rsi handler VA or 0), uptime, chdir, getcwd, sync.
- Userland C: `hi`, `sh`, `init`, `fbhello`, CRT stubs, 8×8 font helper, `memcpy` / `strlen` / `strcmp`. Packed `catch` installs SIGINT and sleeps.
- No job control (`fg` / `bg` / `jobs`), no FAT16, no serial console, no SMP, no networking, no full POSIX signals.

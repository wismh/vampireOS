...
- Syscalls through `int 0x30`: write, exit, yield, sleep, wait/waitpid, open (rsi 1 creates, rsi 2 creates and appends), close, read, readdir, exec, pipe, brk, fork, dup2, lseek, stat, kill, mmap (rsi length rounded up to pages, rdx a file fd copies that file into the mapping), munmap, mprotect (28; rdi VA, rsi length, rdx PROT_READ or PROT_READ|PROT_WRITE), fbinfo, fbpix, fbpresent, sigaction (27; rdi signo, rsi handler VA or 0), uptime, date (29; rdi buf, rsi max; copies `YYYY-MM-DD HH:MM:SS` from the CMOS RTC), chdir, getcwd, sync.

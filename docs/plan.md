...
- Volume: FAT12, 1024 data clusters, four sectors per FAT (both copies). 8.3 names plus VFAT LFN (`ls` / `open` / `cat longname.txt`; `put` / `cp` / `>` of a longer name writes LFN + 8.3 alias). Boot PCI-scans for virtio-blk (vendor `0x1AF4`, device `0x1001` or modern `0x1042`), virtqueue-reads LBA 0, and prints `virt 55aa`. FAT reads/writes use VirtIO when that device probed (first volume write prints `virt wr`); else AHCI DMA when a controller probed (PCI class 0x0106, boot prints `ahci 55aa`, first volume write prints `ahci wr`); else ATA PIO. BIOS still boots from IDE; QEMU attaches the same image to AHCI and to `-device virtio-blk-pci` without `snapshot=on` (`file.locking=off`, `cache=writethrough`).
...
- No job control (`fg` / `bg` / `jobs`), no FAT16, no block device table / `devs`, no partitions, no serial console, no SMP, no networking, no signals beyond kill/Ctrl+C.

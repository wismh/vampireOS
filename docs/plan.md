...
- Syscalls through `int 0x30`: write, exit, yield, sleep, wait/waitpid, open (rsi 1 creates, rsi 2 creates and appends), close, read, readdir, exec, pipe, brk, fork, dup2, lseek, stat, kill, mmap (rdx a file fd copies that file into the page), munmap, fbinfo, fbpix, fbpresent, sigaction (27; rdi signo, rsi handler VA or 0), uptime, chdir, getcwd, sync.
